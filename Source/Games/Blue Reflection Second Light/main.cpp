...
public:
   ...
   DrawOrDispatchOverrideType OnDrawOrDispatch(ID3D11Device* native_device, ID3D11DeviceContext* native_device_context, CommandListData& cmd_list_data, DeviceData& device_data, reshade::api::shader_stage stages, const ShaderHashesList<OneShaderPerPipeline>& original_shader_hashes, bool is_custom_pass, bool& updated_cbuffers, std::function<void()>* original_draw_dispatch_func) override
   {
      ...
      if (original_shader_hashes.Contains(shader_hashes_effect_merge))
      {
         ...
         HRESULT hr = resource->QueryInterface(game_device_data.depth_texture.put());
         if (FAILED(hr))
         {
      ...
      if (game_device_data.frame_phase == FramePhase::FORWARD)
      {
         ...
         {
            reshade::log::message(reshade::log::level::info, "Get depth srv in forward.");
   ...
   static void OnExecuteSecondaryCommandList(reshade::api::command_list* cmd_list, reshade::api::command_list* secondary_cmd_list)
   {
      ...
      if (native_device_context)
      {
         ...
         if (native_command_list.get() == game_device_data.remainder_command_list && game_device_data.partial_command_list)
         {
            ...
            {
               reshade::log::message(reshade::log::level::error, "SR: Replaying search mode.");
            ...
            if (!game_device_data.source_color || !game_device_data.depth_texture || device_data.sr_type == SR::Type::None)// || test_index == 5)
            {
               ...
               reshade::log::message(reshade::log::level::error, "SR: no source color/depth texture.");
            ...
            {
               reshade::log::message(reshade::log::level::error, "SR: Update settings.");
            ...
            */
            
            {
               ...
               reshade::log::message(reshade::log::level::error, "SR: Try draw.");
               reshade::log::message(reshade::log::level::error, std::format("SR: Game Width: {}, Game Height: {}, RenderWidth: {:2f}, RenderHeight: {:2f}.", width, height, device_data.render_resolution.x, device_data.render_resolution.y).c_str());
               reshade::log::message(reshade::log::level::error, "SR: Immediate context: " + (native_device_context->GetType() == D3D11_DEVICE_CONTEXT_IMMEDIATE) ? "True" : "False");
               
               {
                  std::stringstream s;
                  s << "jitter_x = " << draw_data.jitter_x << ", ";
                  s << "jitter_y = " << draw_data.jitter_y << ", ";
                  s << "vert_fov = " << draw_data.vert_fov << ", ";
                  s << "near_plane = " << draw_data.near_plane << ", ";
                  s << "far_plane = " << draw_data.far_plane << ", ";
                  s << "reset = " << draw_data.reset;
               ...
               {
                  game_device_data.source_color->GetDesc(&tex_desc);
                  std::stringstream s;
                  s << "Source Color: ";
                  s << "format = " << tex_desc.Format << ", ";
                  s << "width = " << tex_desc.Width << ", ";
                  s << "height = " << tex_desc.Height;
                  reshade::log::message(reshade::log::level::info, s.str().c_str());
               }
               {
                  game_device_data.resolve_texture->GetDesc(&tex_desc);
                  std::stringstream s;
                  s << "Resolve Color: ";
                  s << "format = " << tex_desc.Format << ", ";
                  s << "width = " << tex_desc.Width << ", ";
               ...
               {
                  game_device_data.decoded_motion_vectors->GetDesc(&tex_desc);
                  std::stringstream s;
                  s << "Motion Vector: ";
                  s << "format = " << tex_desc.Format << ", ";
                  s << "width = " << tex_desc.Width << ", ";
               ...
               {
                  game_device_data.depth_texture->GetDesc(&tex_desc);
                  std::stringstream s;
                  s << "Depth: ";
                  s << "format = " << tex_desc.Format << ", ";
                  s << "width = " << tex_desc.Width << ", ";
               ...
               if (dlss_succeeded)
               {
                  reshade::log::message(reshade::log::level::error, "SR: Copying to source color.");
                  native_device_context->CopyResource(game_device_data.source_color.get(), game_device_data.resolve_texture.get());
                  reshade::log::message(reshade::log::level::error, "SR: Draw success.");
               }
            }
            ...
            
            {
               ...
               reshade::log::message(reshade::log::level::error, "SR: Draw TAA depth.");
               game_device_data.temporal_depth_pass.Draw(native_device.get(), native_device_context.get(), device_data, draw_data);
               reshade::log::message(reshade::log::level::error, "SR: After draw TAA depth.");
            }
   ...
   static bool OnClearRenderTargetView(reshade::api::command_list* cmd_list, reshade::api::resource_view rtv, const float color[4], uint32_t rect_count, const reshade::api::rect* rects)
   {
      ...
      if (game_device_data.frame_phase == FramePhase::NONE && game_device_data.source_color_rtv.get() == nullptr)
      {
         ...
         if (rtv_desc.Format == DXGI_FORMAT_R11G11B10_FLOAT)
         {
            ...
            {
               reshade::log::message(reshade::log::level::info, std::format("CameraData: {:2f}.", CameraData->camera_distances.x).c_str());
